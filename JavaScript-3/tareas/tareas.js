...
function numeroMasGrande(numeros) {
  ...
  // Tu código:
  numeros  = Math.max.apply(null,numeros);
  return numeros;
}
...
function cuentoElementos(arreglo){
  ...
  const cantidadArreglos = []
  for (var i = 0; i < arreglo.length; i++){
    if (arreglo[i] > 19 ){
      cantidadArreglos.push(arreglo[i])
    }
  }
  return cantidadArreglos.length;
}
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  let numeroenstring= String(n)
  return numeroenstring[0] == 9;
